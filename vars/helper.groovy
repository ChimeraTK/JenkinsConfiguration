...
if [ -f "/scratch/source/\${RUN_FROM_SUBDIR}/CMakeLists.txt" ]; then
  ...
  if [ -z \\\${INSTALL_PREFIX} ]; then
    INSTALL_PREFIX=/export/doocs
    LIBDIR=lib
  else
    LIBDIR=${label == "tumbleweed" ? "lib64" : "lib"}
  fi
  meson setup /scratch/source/\${RUN_FROM_SUBDIR} --wrap-mode=nofallback --buildtype=${buildType == "Debug" ? "debug" : "debugoptimized"} --prefix=\\\${INSTALL_PREFIX} --libdir \\\${LIBDIR} --includedir \\\${LIBDIR}/include -Db_lundef=false
else
  echo "*********************************************************************"
