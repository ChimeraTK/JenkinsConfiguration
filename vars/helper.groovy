...
def doDependencyArtefacts(ArrayList<String> dependencyList, String label, String buildType, ArrayList<String> obtainedArtefacts=[]) {
  ...
  script {
    ...
    dependencyList.each {
      ...
      // unpack artefact
      sh """
        tar xf \"artefacts/install-${dependency_cleaned}-${label}-${buildType}.tgz\" -C / --keep-directory-symlink --use-compress-program=pigz
      """
  ...
  script {
    ...
    sh """
      for a in artefacts/build-*-${label}-${buildType}.tgz ; do
        sudo -H -E -u msk_jenkins tar xf \"\${a}\" -C / --use-compress-program=pigz
      done
  ...
  sh """
    if ls artefacts/install-*-${label}-${buildType}.tgz 1>/dev/null 2>&1; then
      for a in artefacts/install-*-${label}-${buildType}.tgz ; do
        tar xf \"\${a}\" -C / --use-compress-program=pigz
      done
    fi
  """
...
EOF
  ...
  script {
    ...
    sh """
      sudo -H -E -u msk_jenkins tar cf build-${JOBNAME_CLEANED}-${label}-${buildType}.tgz /scratch --use-compress-program=pigz
    """
...
def doInstall(String label, String buildType) {
  ...
  sh """
    ...
    sudo -H -E -u msk_jenkins tar cf ${WORKSPACE}/install-${JOBNAME_CLEANED}-${label}-${buildType}.tgz . --use-compress-program=pigz
  """
